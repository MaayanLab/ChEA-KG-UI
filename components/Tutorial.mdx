...
Submitted gene sets are stored in the database for persistent link purposes only. These gene sets are linked to a unique UUID which are kept private unless the users share the persistent link with anyone.