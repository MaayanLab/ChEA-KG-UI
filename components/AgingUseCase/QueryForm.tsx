...
const QueryForm = ({
	...
	const [agingTypes, setagingTypes] = useState(aging_gmt)
	...
	useEffect(()=>{
        ...
        for (const [k, v] of Object.entries(aging_gmt)) {
    ...
    useEffect(()=>{
		if (index) {
			if (fullTextQuery === '') setagingTypes(agingTypes)
			else {
				const new_vals = {}
				for (const key of index.search(`*${fullTextQuery}*`)) {
					...
					new_vals[group_name][label] = aging_gmt[group_name][label]
				}
				setagingTypes(new_vals)
			}
		}
    }, [fullTextQuery])
        ...
        <FormGroup>
            ...
            <Grid container spacing={2}>
				<Typography variant="body1" sx={{marginLeft: 2}}>
                    ...
                    <Box>
                        ...
                        >
                            ...
                            title="Choose an aging tissue network"
                    ...
                    {(v === "2") && <Grid item xs={12} md={12}>
                        ...
                        <Stack direction= "column" alignItems="center" justifyContent={'space-around'} spacing={2}>
                                <Grid sx={{width: "100%"}}>
                                    ...
                                    <br></br>
                                    <p style={{fontSize: '12px', textAlign:'left'}}> The <b>{group_name}</b> consensus gene sets were constructed using <u>{aging_info[`${group_name}:up`].num_sigs}</u> signatures, representing <u>{aging_info[`${group_name}:up`].drug_list.length}</u> unique drugs.</p>
                                    </div>
                                    
                                </Grid>
                                {/* <Grid alignContent={'center'}>
                                    <Link target="_blank" rel="noopener noreferrer" href={aging_info[`${group_name}:${term}`].enrichr_url}> 
                                        <Button
                                        size="small"
                                        variant="contained"
                                        color='primary'
               ...
               <Stack direction="column" spacing={2} sx={{justifyContent:"flex-start", paddingTop:2}}>
					<AgingSelector elements={elements} aging_gmt={agingTypes} group_name={group_name} term={combined_query.term} aging_info={aging_info}/>
