...
const Enrichment = async ({
    ...
    console.log("Atlas schema fetched")
        const aging_info = {}
        for (const i of atlasschema.aging_tissues){
        aging_info[i.term] = {
        }
    } 
    ...
    try {
        ...
        const aging_gmt = await (await fetch(`${process.env.NEXT_PUBLIC_HOST}${process.env.NEXT_PUBLIC_PREFIX ? process.env.NEXT_PUBLIC_PREFIX: ""}/api/enrichment/get_aging_gene_sets`)).json()
        
        const default_group = props.default_options.group_name || Object.keys(aging_gmt)[0]
        const default_term = props.default_options.term || Object.keys(aging_gmt[default_group])[0]
        ...
        if (term !==undefined && group_name !== undefined) {
            ...
            const genes = aging_gmt[group_name][term]
        ...
        return (
            <Grid container spacing={2} >
                ...
                <Grid item xs={12} md={3}>
                    <QueryForm 
                        genes={aging_gmt[group_name][term]}
                        ...
                        <Card sx={{borderRadius: "24px", minHeight: 450, width: "100%"}}>
                            <CardContent>
                                {(userListId === undefined || (term === undefined && group_name === undefined)) ?
                                    ...
                                    <> 
                                        {input_desc && (() => {
                                            const [term, dir] = input_desc.split(":")
                                            return (
